...
class uniformpairwisedisjointrandomgraph : public pairwisedisjointrandomgraph {
public:
    std::string shortname() override {return "rpd1";}

    virtual void randomgraph( graphtype* gptr, graphtype* parentg, std::vector<int>* subg, bool* availv ) {
        ...
        while (d < dim)
        {
            while (rvertex < 0 || (rvertex >= 0 && (subgbool[rvertex] || !availv[rvertex])))
                rvertex = (int)((dist10000(rng) * (double)(pdim))/RANDOMRANGE);
            subgbool[rvertex] = true;
            availv[rvertex] = false;
            ++d;
        }
        int r = 0;
        int s;
    ...
    std::vector<std::vector<graphtype*>> subrandomgraphsinternal(std::vector<int> dims, graphtype* parentg, std::vector<int>* subg, const int cnt) {
        std::vector<std::vector<graphtype*>> res {};

        res.resize(cnt);
        for (int i = 0; i < cnt; ++i) {
            bool* availv = (bool*)malloc(parentg->dim * sizeof(bool));
            for (int j = 0; j < parentg->dim; ++j)
                availv[j] = true;
            for (int j = 0; j < dims.size(); ++j) {
                graphtype* rg = new graphtype(dims[j]);
                randomgraph(rg,parentg,subg,availv);;
                res[i].push_back(rg);
            }
            delete availv;
        }
    ...
    std::vector<std::vector<graphtype*>> randomgraphs( std::vector<int> dims, graphtype* parentg, std::vector<int>* subg, const int cnt ) override {
        ...
        for (int j = 0; j < thread_count; ++j) {
            ...
            t[j] = std::async(&uniformpairwisedisjointrandomgraph::subrandomgraphsinternal,this,dims,parentg,subg,stopidx-startidx);
        }
        std::vector<std::vector<std::vector<graphtype*>>> gvv {};
    ...
    uniformpairwisedisjointrandomgraph() : pairwisedisjointrandomgraph("uniform pairwise disjoint random") {}
