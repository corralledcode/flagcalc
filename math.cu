...
public:
    ...
    bool checkGPUquantforfast(formulaclass& fc, uint& num, uint& sz)
    {
        ...
        num = 0;
        for (int i = 0; i < GPUQUANTFASTDIM; ++i)
            if (fc.fcright->boundvariables.size() > num)
            {
                valms v = emf->evalinternal(*fc.fcright->boundvariables[fc.fcright->boundvariables.size()-num-1]->superset,
                    context);
                c[num++] = {fc.fcright->boundvariables[fc.fcright->boundvariables.size()-num-1]->name, v};
            }
        c.resize(num);
        int i = 0;
        ...
        if (i == j)
        {
            ...
            num = i;
            for (int k = 0; k < num; ++k)
                fc.fcright->boundvariables[k]->CUDAfastidx = k;
            return true;
        }
    }
};
